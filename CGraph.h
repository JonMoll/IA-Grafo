...
{
    cout<<"Grafo"<<endl;
    cout<<"Nodos:";for(int i=0; i<m_nodes.size();i++)
    ...
    for(int i=0; i<m_edges.size(); i++)
    {
        ...
        if(!m_edges[i]->m_dir) 
            ...
            cout<<"  -- "<<m_edges[i]->m_data;
            cout<<" -->   "<<m_edges[i]->m_node[0]->m_data<<endl;
            }
    }
}
...
template <class Tr>
bool CGraph<Tr>::insert_edge(E _data,N a,N b,bool dir){
    Node *Nodea,*Nodeb;
    for(int i=0; i<m_nodes.size(); i++){
        if(m_nodes[i]->m_data==a) Nodea=m_nodes[i];
        if(m_nodes[i]->m_data==b) Nodeb=m_nodes[i];
    }
    Edge* new_edge=new Edge(_data,Nodea,Nodeb,dir);
